...
module TestOracles =
 struct
  ...
  let oracle_parent_child i =
    let left = ExtractedModels.merkle_left_child i in
    let right = ExtractedModels.merkle_right_child i in
    (&&) (Z.eqb (ExtractedModels.merkle_parent left) i)
      (Z.eqb (ExtractedModels.merkle_parent right) i)

  (** val oracle_sibling_involutive : int -> bool **)

  let oracle_sibling_involutive i =
    if Z.ltb i ((fun x -> x) 1)
    then true
    else Z.eqb
           (ExtractedModels.merkle_sibling (ExtractedModels.merkle_sibling i))
           i

  (** val oracle_model_select : int -> int -> bool **)

  let oracle_model_select a b =
    (&&) (Z.eqb (ExtractedModels.model_select a b ((fun x -> x) 1)) a)
      (Z.eqb (ExtractedModels.model_select a b 0) b)

  (** val oracle_ct_select : int -> int -> bool **)

  let oracle_ct_select =
    oracle_model_select

  (** val oracle_nonce_roundtrip : int -> int -> bool **)

  let oracle_nonce_roundtrip key_id counter =
    if Z.ltb key_id 0
    then true
    else if Z.ltb counter 0
         then true
         else if Z.leb
              ...
              then true
                        ...
                        counter
                   then true
                   else let idx = ExtractedModels.nonce_index key_id counter
                        in
                        (&&)
                          (Z.eqb (ExtractedModels.nonce_key_id idx) key_id)
                          (Z.eqb (ExtractedModels.nonce_counter idx) counter)

  (** val oracle_threshold_monotonic : int -> int -> int -> bool **)

  let oracle_threshold_monotonic threshold n1 n2 =
    if Z.leb n2 n1
    then true
    else if ExtractedModels.valid_threshold threshold n1
         then ExtractedModels.valid_threshold threshold n2
         else true
 end
